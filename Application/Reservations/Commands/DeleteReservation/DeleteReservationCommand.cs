        ...
        public async Task<Unit> Handle(DeleteReservationCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!isOwner && !isAdmin)
            {
                ...
                if (reservation.Status.Status == Status.Accepted)
                {
                    ...
                    {
                        throw new Exception("You cannot delete reservations that have passed");
                    }
                }

                if (reservation.Date == today || (reservation.Date.AddDays(-1) == today && reservation.StartTime < time))
                {
                    throw new Exception("You cannot cancel a reservation 24 hours before the start of the appointment. Contact the sports facility: " + sportObject.Phone);
                }
            } 
